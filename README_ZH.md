...
- **轨迹文件** (`.xtc`, `.trr`) - GROMACS 轨迹文件，支持 Mol* 3D 查看器和流式加载
...
| 轨迹文件 | `.xtc`, `.trr` | GROMACS 轨迹文件 | Mol* 3D 查看器、流式加载、多帧播放 |
...
### 🎬 Mol* 轨迹查看器
- **3D 分子可视化**：使用 Mol* 库直接在 VS Code 中预览 XTC 和 TRR 轨迹文件
- **流式轨迹加载**：高效加载超大型轨迹文件，按需加载帧数据
  - 显著降低内存占用，按需加载帧数据
  - 支持远程轨迹文件的流式传输
  - 自动建立帧索引，实现快速帧定位
  - 适用于 GB 级别的长期模拟轨迹
- **多面板支持**：同时打开多个 Mol* 查看器，每个按文件路径独立管理
- **状态持久化**：查看器状态在 VS Code 重启后自动恢复

**使用方法：**
1. 右键单击资源管理器中的 `.xtc` 或 `.trr` 文件
2. 选择 "Open with Mol* Viewer"
3. 选择或确认拓扑文件（`.gro` 或 `.pdb`）
4. 使用播放控件浏览轨迹帧
...
#### 🚀 优化与修复
- **拓扑解析优化**：大幅提升大型拓扑文件的解析性能（采用 O(N) 单次遍历算法）。
- **远程监控修复**：修复了远程监控脚本的部署路径问题。

### [0.3.2] - 2025-12-24

#### 🔧 改进
- 改进 MDP 文件格式化功能，对齐和缩进更加智能。
- 为 MDP 参数提示添加了多语言翻译。
